...
// 3. import the classnames library
...
class AddContact extends Component {
  ...
  onSubmit = async(dispatch, e) => {
    ...
    // Check for errors.
    // 4. we can add in our validation here.
    ...
    // 5. Check for errors in the name field.
    if (name === ''){
      // 6. this sets errors.name state value
      this.setState({ errors: {name: 'Name is required '}});
      return; // 7. this will stop the onSubmit from running
    }
    // 8. Check for errors in the email field
    if (email === ''){
      // 9. this sets errors.name state value
      this.setState({ errors: {email: 'Email is required '}});
      return; // 10. this will stop the onSubmit from running
    }
    // 11. Check for errors in the phone field
    if (phone === ''){
      // 12. this sets errors.name state value
      this.setState({ errors: {phone: 'Phone is required '}});
      return; // 13. this will stop the onSubmit from running
    }
