...
export class Provider extends Component {
  ...
  // componentDidMount is a lifecycle method. This code will execute after the component has mounted.
  // All data in React is handled with async functions.
  async componentDidMount(){
    // 7. use axios.get to an API endpoint to get data from an API. We are changing this to get data from our API now.
    // The endpoint we need to hit is /api/contacts to get all contacts.
    const res = await axios.get('/api/contacts');
    console.log(res.data);
    // Set the state from our response from the API
    this.setState({ contacts: res.data})
  };
  // End of componentDidMount()
  // 8. open up server/src/server.js
