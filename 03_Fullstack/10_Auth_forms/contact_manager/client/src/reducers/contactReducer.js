...
// import our types from actions/types.js
import {
  GET_CONTACTS,
  GET_CONTACT,
  ADD_CONTACT,
...
// The reducer for our contacts actions and how it will update the state.
export default function contactReducer(state = initialState, action) {
  switch(action.type) {
    ...
    // 6. update contact.id to contact.C_id
    case UPDATE_CONTACT: 
      return {
        ...state, 
        contacts: state.contacts.map(contact => 
          contact.C_id === action.payload.id 
    ...
    // 7. change contact.id to contact.C_id
    case DELETE_CONTACT: 
      return {
        ...state, 
        contacts: state.contacts.filter(
          contact => contact.C_id !== action.payload
        ),
        loading: false
        // The filter will remove the contact from the contacts array.
        // return the current state plus the new contacts array.
        // 8. open src/actions/contactActions.js
      }
